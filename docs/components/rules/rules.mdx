...
| Rule                                     |                                                                                                        |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `@basketry/rules/lib/no-null-properties` | Requires that properties may not be null. When this rule is applied, properties may still be optional. |

#### Options

By default, this rule is applied to all properties for all input and output types. This may be overridden with the `allowHttpVerbs` option to allow input types for certain HTTP verbs to be exempt from this rule.

```json
{
  "rules": [
    {
      "rule": "@basketry/rules/lib/no-null-properties",
      "options": { "allowHttpVerbs": ["PATCH"] }
    }
  ]
}
```
...
```json
{
  "rules": [
    {
      "rule": "@basketry/rules/lib/pluralization",
      "options": { "severity": "warning" }
    }
  ]
}
